...
const darkThemeOverrides = {
  ...
  'color-bg-default': primitiveTokens.color.gray[900],
  'color-bg-surface': primitiveTokens.color.gray[800],
  'color-bg-elevated': primitiveTokens.color.gray[700],
  'color-bg-inverse': primitiveTokens.color.gray[50],
  
  // Text (inverted)
  'color-text-primary': primitiveTokens.color.gray[50],
  'color-text-secondary': primitiveTokens.color.gray[400],
  ...
  // Actions (same primary, different hover states)
...
export function generateThemeCSS(theme = 'light') {
  ...
  // Root selector for light theme, .dark for dark theme
  const selector = theme === 'dark' ? ':root.dark' : ':root';
  cssLines.push(`${selector} {`);
  
  // Flatten primitive tokens
  const flatPrimitives = flattenTokens(primitiveTokens, 'color');
  ...
  for (const [key, value] of Object.entries(flatSemanticColors)) {
    // Apply dark theme overrides if applicable
    const overrideKey = key;
    const cssValue = theme === 'dark' && darkThemeOverrides[overrideKey]
      ? darkThemeOverrides[overrideKey]
      : value;
    
    cssLines.push(`  --${key}: ${cssValue};`);
  }
...
export function generateTokenDocumentation() {
  ...
  lines.push('### Colors\n');
  lines.push('```css');
  const flatPrimitives = flattenTokens(primitiveTokens.color, 'color');
  for (const [key, value] of Object.entries(flatPrimitives)) {
