...
for (const d of [-0.1,-0.2,-0.25,-0.28,-0.294,-0.3,-0.35,-0.38,-0.4,-0.5]) {
  const r = await engine.processPath(path5, d, { trimSelfIntersections: false });
  const c = r.contours[0];
  const arcs = c?.segments.filter(s=>s.type==="arc").length ?? 0;
  const segs = c?.segments.length ?? 0;
  console.log(`d=${d} contours=${r.contours.length} segs=${segs} arcs=${arcs}`);
