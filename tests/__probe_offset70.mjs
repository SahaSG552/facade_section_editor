...
segs.forEach((s, i) => {
  const len =
    s.type === "line"
      ...
      : "(arc)";
  console.log(
    `  [${i}] ${s.type}  ${s.start.x.toFixed(4)},${s.start.y.toFixed(4)} -> ${s.end.x.toFixed(4)},${s.end.y.toFixed(4)}  ${s.type === "line" ? "len=" + len.toFixed(3) : ""}`,
  );
});
console.log("");
  ...
  try {
    ...
    for (const s of result) {
      if (s.type === "line") {
        ...
        console.log(
          `  L  ${s.start.x.toFixed(3)},${s.start.y.toFixed(3)} -> ${s.end.x.toFixed(3)},${s.end.y.toFixed(3)}  len=${len.toFixed(3)}${flag}`,
        );
      } else if (s.type === "arc") {
