...
for (const d of [1, 2, 3, 4]) {
  ...
  console.log(
    `d=${d}: arc r=${r.toFixed(4)}, start=(${startX},${startY}), end=(${endX?.toFixed(4)},${endY?.toFixed(4)})`,
  );
  if (
    endY !== null &&
    Math.abs(startX - endX) < 1e-9 &&
    Math.abs(startY - endY) < 1e-9
  ) {
...
for (const d of [1, 2, 3]) {
  console.log(`\n--- d=${d} ---`);
  const res = buildOffsetContour(makeSegments(), d, {
    joinType: "sharp",
    capType: "flat",
    skipCap: true,
  });
  res.forEach((s, i) => {
    const r = s.arc
      ? ` r=${s.arc.radius?.toFixed(4)} center=(${(s.arc.centerX ?? s.arc.center?.x)?.toFixed(4)},${(s.arc.centerY ?? s.arc.center?.y)?.toFixed(4)})`
      : "";
    console.log(
      `  seg[${i}] ${s.type} (${s.start.x.toFixed(4)},${s.start.y.toFixed(4)}) -> (${s.end.x.toFixed(4)},${s.end.y.toFixed(4)})${r}`,
    );
  });
}
