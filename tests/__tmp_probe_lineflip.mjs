...
for (const d of [
  -0.3, -0.2, -0.15, -0.12, -0.1, -0.08, -0.06, -0.05, -0.04, 0.04, 0.05, 0.06,
  0.08, 0.1, 0.12,
]) {
  const r = await engine.processPath(path4, d, {
  ...
  const tiny =
    segs
      ...
      .sort((a, b) => a - b)[0] ?? 0;
  const match =
    r.pathData?.includes("8.300000 1.717") ||
    r.pathData?.includes("8.300000 1.7177");
  console.log(
    `d=${d} segs=${segs.length} tiny=${tiny.toFixed(6)} match=${match}`,
  );
