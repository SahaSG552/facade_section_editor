...
updated: 2026-04-09T01:37:00Z
---

## Current Focus

hypothesis: Корень shape mismatch подтвержден и исправлен: знак sweep-зависимого вклада в offsetArc был инвертирован
test: Прогнать релевантный набор на cursor-side и reverse-shape invariance для user-case
expecting: Оба направления line-arc-line дают эквивалентную форму и корректную сторону к курсору
next_action: выполнить финальный targeted прогон и запросить user verification в UI
...
root_cause: В one-sided режиме открытых контуров знак дистанции после cursor-side резолвинга повторно переопределялся через signedArea (orientation-dependent), из-за чего реверс направления контура мог инвертировать сторону смещения.
fix: 1) В OffsetEngine open one-sided сделан условный выбор знака (cursor-side mode vs legacy). 2) В OffsetTool прокинуты sideResolution/cursorPoint в runtime вызов OffsetEngine для open контуров, с корректным переводом cursorPoint в path-space.
verification: "Targeted: tests/offset-open-line-arc-reverse-shape-invariance.spec.js + tests/offset-cursor-direction-invariance.spec.js + tests/offset-cursor-side.spec.js => 6/6 passed; требуется UI-подтверждение пользователем"
files_changed: ["src/operations/OffsetEngine.js", "src/editor/tools/OffsetTool.js", "tests/offset-cursor-direction-invariance.spec.js"]
