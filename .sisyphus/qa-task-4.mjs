...
const { trimSelfIntersections, segmentsToPathString, pathStringToSegments } =
  OffsetTrimmer;
...
selfIntersectingSegments.forEach((seg, i) => {
  console.log(
    `  ${i}: ${seg.type} (${seg.start.x},${seg.start.y}) → (${seg.end.x},${seg.end.y})`,
  );
});
...
cleanSegments.forEach((seg, i) => {
  if (seg.type === "line") {
    console.log(
      `  ${i}: ${seg.type} (${seg.start.x.toFixed(2)},${seg.start.y.toFixed(2)}) → (${seg.end.x.toFixed(2)},${seg.end.y.toFixed(2)})`,
    );
...
const testSegments = [
  { type: "line", start: { x: 0, y: 0 }, end: { x: 5, y: 0 } },
  {
    ...
    arc: { radius: 5, sweepFlag: 1, largeArcFlag: 0 },
  },
